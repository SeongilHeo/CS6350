...
class ID3:
    ...
    Methods:
        train(data, labels, attributes, columns, numerical_attributes=[])
        _build_tree(data, labels, attributes, depth)
        _split_data(data, labels, attribute)
        _best_split(data, labels, attributes)
    ...
    def train(self, data, labels, attributes, columns, numerical_attributes=[]):
        ...
        self.root = self._build_tree(data, labels, attributes=list(self.attributes.keys()), depth=0)

    def _build_tree(self, data, labels, attributes, depth):
        """    
        ...
        best_attribute, best_threshold = self._best_split(data, labels, attributes)
        ...
        subsets = self._split_data(data, labels, best_attribute)
        ...
        for attribute_value, (subset_data, subset_labels) in subsets.items():
            if len(subset_labels) == 0:
                ...
                child_node = self._build_tree(subset_data, subset_labels, child_attribute, depth + 1)
    ...
    def _split_data(self, data, labels, attribute):
        ...
        Returns:
            dict: 'keys': attribute values (0 and 1 for numerical attributes).
                  'values': tuple. (list of subset data, list of subset labels).
        """
        # numerical attribute
    ...
    def _best_split(self, data, labels, attributes):
        ...
        for attribute in self.attributes.keys():
            ...
            weighted_avg = 0.0
            for subset_data, subset_labels in splits.values():
